...
\begin{abstract}
We present 83 fast radio bursts localized to 2'' by $\sim 1'$ precision by the Canadian Hydrogen Intensity Mapping Experiment (CHIME), as well as 18 new host galaxies, and 14 spectroscopic redshifts.
\end{abstract}
...
The PATH framework accounts for the survey depth mismatch by specifying the ``unseen'' probability $P(U)$, which uses the FRB redshift distribution, the galaxy luminosity function, the localization ellipse size, and the optical survey depth to calculate the prior probability that the host galaxy of an individual FRB in a survey remains. For the specific combination of FRBs detected by CHIME, we assign $P(U) = 0.32$ for the PanSTARRS-PS1 fields, and $P(U) = 0.15$ for the DECaLS fields. We note that this is a conservative choice because the sample of bursts presented here represent the brightest bursts detected by CHIME, and hence preferentially sample the nearby end of the CHIME redshift distribution~\citep{shin2023inferring}. We reserve a detailed justification of these choices to an upcoming simulation paper~\citet{andersen2024inprep}, 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/hg_vs_dm.pdf}
    \caption{Caption}
