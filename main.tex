...
We use the default priors in the PATH toolkit to assign posterior probabilities to each catalogued galaxy allowed by the combined localization region from baseband and CHIME-KKO VLBI. Each localization region is cross-matched with a widefield galaxy survey. Where possible (55/78 fields), we cross-match against the Dark Energy Camera Legacy Survey (DECaLS) to find a host galaxy due to its greater depth. In the remainder of fields (23/78), we default to Pan-STARRS PS1, which covers the entire CHIME/FRB discovery footprint. \textbf{Note: Waiting for last 5 fields from custom PATH; update these numbers.}

Stars are identified in each field on the basis of their Gaia DR3 classification. After automated removal, each field is manually inspected for contamination by misclassified stars \textbf{and other failures}. Finally, each galaxy candidate in the field is assigned a probability of being the true host.
