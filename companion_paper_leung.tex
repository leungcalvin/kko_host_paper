...
In Eq.~\ref{eq:dmh_definition}, the subscripts ``tot'', ``cosmic'', and ``MW'' stand for the measured total DM, the cosmic contribution (as implemented in the FRBs repo), and the Galactic component, which in turn includes a subcomponent for the interstellar medium (ISM) and halo. The angle brackets in this paper refer to an average over all sightlines.

We use a new low-redshift catalog of FRBs, presented in a companion work~\citep{chime2024catalog}, and combine it with localized FRB in the literature to make an accurate measurement of the average gas column in the vicinity of the host galaxy as quantified by $\langle \dmh \rangle$, as well as higher moments thereof ($\langle \dmh^2 \rangle$, etc.). These moments non-parametrically specify the sightline-to-sightline variation in $\dmh$, and contain information about the spatial distribution of ionized gas relative to that of FRB hosts~\citep{cordes2016radio, reischke2024analytical}.

Finally, we point out that the $\dmh$ versus stellar mass relation can constrain models for galaxy formation and galactic winds from e.g. supernova and AGN feedback at halo masses formerly inaccessible using traditional gas probes, i.e. X-ray emission and Sunyaev-Zeldovich observations. We use this to rule out models for galaxy formation and evolution as implemented within the CAMELS suite of hydrodynamical simulations.

In \S~\ref{sec:sample} we describe how we chose, from over a hundred FRB hosts in the literature, the final sample of sightlines which we use in this paper.
In \S~\ref{sec:dmh} we describe our measurement of the host galaxy contribution to the DM budget using our sample. This is the first time this has been done in the regime where redshift evolution is negligible.
Finally in \S~\ref{sec:camels} we introduce the \dmh-stellar mass relation and compare it to theoretical predictions from three sub-grid models implemented in the CAMELS suite of simulations, implying a strong feedback universe.
...
In Fig.~\ref{fig:host_dm} we have compiled the vast majority of FRBs with secure redshifts and publicly-available photometry detected to date. The combined sample consists of 53 sightlines at $z < 0.2$ (Fig.~\ref{fig:host_dm}). Next, we perform a redshift cut ($z < 0.2$), and exclude from our analysis non-luminous ($M_r > -17$) and the single remaining dwarf hosts \citep[FRB 20121102A; FRB 20190520B and FRB 20210117A are at $z > 0.2$][]{bhandari2023nonrepeating,hewitt2024repeating}. These sources are not representative from the population of detected FRBs; on the basis of the high specific star formation rates of their hosts and persistent radio source counterparts their progenitors may be produced by a formation channel distinct from that of FRBs from more massive galaxies. For this reason, our \dmh\ parameter estimates are valid only for FRBs with non-dwarf hosts over the aforementioned range of host stellar masses. Finally, we identify the following critical subsamples of FRBs within the remaining 49 FRBs.
\begin{enumerate} 
