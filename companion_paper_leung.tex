...
We select a subsample of FRBs available in the literature to reduce contaminants for our measurement of \dmh, out of all FRBs in the literature with secure redshifts and publicly-available photometry. We perform a redshift cut, eliminating sources beyond $z > 0.2$. This boundary is chosen to balance the trade off between reduced contamination from restricting our sample to lower redshifts and better statistics from including more distant sources. The \dmh\ distribution of all FRB sightlines at $z < 0.2$ is shown in Fig.~\ref{fig:host_dm}. Each \dmh\ measurement is associated with some uncertainty which is dominated by astrophysical (not measurement) uncertainties. As mentioned earlier, the difficulty in measuring the CGM component of FRB host galaxies comes from unmodeled contributions along the line of sight. These include contributions as follows:
\begin{enumerate}
    \item DM from large-scale gas overdensities and underdensities which trace the large scale structure, on $\sim$ 100 Mpc scales. While the IGM contribution at low redshift is inhomogeneous, dispersion-measure ray tracing through the Illustris TNG simulation reveals that on average, the root-mean-square contribution from large-scale structure ($\sigma_\textrm{IGM}$) adds 63.3 pc cm$^{-3}$ of RMS scatter at $z = 0.1$ and 105 pc cm$^{-3}$ at $z = 0.2$~\citep{walker2024dispersion}. We account for this in the uncertainties of each \dmh\ measurement by interpolating by  these two RMS error measurements as a function of redshift, but find that applying $\sigma_\textrm{IGM} = 80$ pc cm$^{-3}$ across our sample does not alter our results.
    \item DM from intervening galaxies, groups, and clusters along the line of sight, on $\sim$ Mpc scales. The halo mass function predicts that the rate of intervening halos within their $R_{vir}$ is $< 1$ per sightline at $z < 0.2$ for halos more massive than $10^{12} M_\odot$ ~\citep{mcquinn2014locating}. In addition, the NASA/IPAC Extragalactic Database (NED) Local Volume Sample (NED-LVS) is complete to $L^*$ galaxies out to 400 Mpc~\citep{cook2023completeness}. This allows us to check individual sightlines for significant ($\gtrsim L^*$) interlopers out to $z = 0.1$, (see Fig.~\ref{fig:dm_halos}) and to plot the DM contributions from intervening halos (DM$_\mathrm{halos}$). We find that this contribution out to the completeness horizon of NED-LVS is subdominant; out to twice the distance ($z = 0.2$), DM$_\mathrm{halos}$ is no more than double this on average.
    \item DM from the ISM of the host galaxy, which contaminates the DM on $\sim$ kiloparsec scales. We control for the ISM of the host galaxy by eliminating sightlines with host galaxies more inclined than $i = 70^\circ$. In addition, we impose a magnitude cut ($M_R < -17$) to eliminate dwarf galaxies from our sample, whose high specific star formation rates are associated with high ISM gas densities. 
    \item DM from the ISM of the Milky Way. To account for this, we eliminate sightlines at Galactic latitudes below $|b| < 6^\circ$ and add a 20\% uncertainty on DM$_\mathrm{NE2001}$ to each of our measurements of \dmh.
    \item DM from the circum-source medium of the FRB, on potentially $\sim$ parsec scales. This contribution is unconstrained but unlikely to significantly dominate the others. While scattering timescales may trace source-local DM contributions in some circumstances~\citep{cordes2022redshift}, we use them only as a binary proxy for contamination, excluding sightlines where the FRB signal shows scattering beyond 2 milliseconds at 600 MHz. For this reason, our measurements of \dmh can be thought of as upper limits on the CGM contribution.
\end{enumerate}

Next, we perform a redshift cut ($z < 0.2$). This excludes distant sources including one in an Abell cluster~\citep{connor2023deep}, and two of the three known dwarf hosts~\citep[FRB 20190520B \& 20210117A][]{bhandari2023nonrepeating,hewitt2024repeating}. In addition, selecting only galaxies more luminous than ($M_r > -17$) removes the dwarf host of FRB 20121102A. These sources are not representative from the population of detected FRBs; on the basis of the high specific star formation rates of their hosts and persistent radio source counterparts their progenitors may be produced by a formation channel distinct from that of FRBs from more massive galaxies. With the remaining FRBs in this sample our \dmh\ parameter estimates are valid only for FRBs with non-dwarf hosts over the aforementioned range of host stellar masses. Finally, we identify the following critical subsamples of FRBs within the remaining 49 FRBs.
\begin{enumerate} 
...
Previous studies which involve samples of more distant FRB hosts may be subject to optical selection, in which host galaxies significantly less luminous than $L^*$ may remain identified.~\citep{seebeck2021effects,marnoch2023unseen}, or the assumption that $\langle \dmh \rangle$ does not change over cosmic time.

\section{The Host DM and its Moments}\label{sec:moments}
Our sample of low-redshift sightlines allow us to make the most precise and feedback-insensitive measurement of the distribution of \dmh (see Eq.~\ref{eq:dmh_definition}). 
...
\section{The host contribution to the FRB DM}
Individual DM budgets have been observed to both undershoot ~\citep[FRB 20220319B; see][]{ravi2023deep} and overshoot expectations due to an insufficiently-modeled sightline~\citep[e.g. FRB 20190520A][]{niu2021repeating,lee2023frb}. With every new localized FRB, observers are motivated to dissect in great detail~\citep{2024arXiv240200505K} the DM excess: is \dmh~dominated by a dense circum-source medium, or could it possibly originate from gas along the line of sight? At $z \gtrsim 0.2$ this question is difficult to answer because the contribution of large-scale structure to the DM budget -- the overdensities/underdensities relative to $\langle \textrm{DM}_\text{cosmic}\rangle$ -- dominates over smaller-scale contributions from e.g. the host galaxy and circum-source medium. While estimates are highly uncertain,~\citet{walker2024dispersion} estimate an RMS fluctuation in DM$_\textrm{cosmic}$ of $\gtrsim 100$ pc cm$^{-3}$ -- roughly the size of the mean host contribution -- beyond $z = 0.2$. The scatter DM$_\textrm{cosmic}$ is sufficient to swamp the DM contribution from most circum-source media and Milky Way-like electron disks at all but the most edge-on inclinations~\citep{cassanelli2024fast}. 

At lower redshifts, this question is more tractable since the mean and the large-scale structure contributions 
to the DM budget is minimized ($\lesssim 100$ \,pc~cm$^{-3}$). At a low redshift and above a certain DM threshold ($z < 0.2$, DM$_{eg} \gtrsim 200$ pc cm$^{-3}$), the discrete structures responsible for DM excesses and deficits may be identifiable in archival surveys with little or no casework. This is because the probability of encountering multiple line-of-sight structures is $\ll 1$ for halo masses exceeding $>10^{12} M_\odot$~\citep[Fig.~1 of][]{mcquinn2014locating}, whereas most FRB sources live in galaxies with stellar masses of $10^{10-11} M_{\odot}$ (again, notable exceptions exist~\citep{tendulkar2017host,niu2021repeating,bhandari2023nonrepeating}). For FRBs originating in $L^*$ haloes at low redshifts, the dominant contributions to the DM budget come from the host galaxy. Fig.~\ref{fig:host_dm} summarizes our efforts to identify sources of large DM excesses in this pristine regime where the host galaxy contribution may be measured with little line-of-sight contamination.
...
\section{The DM-host stellar mass relation}\label{sec:camels}
