...
\begin{enumerate}
    \item The cosmic contribution, which traces the large-scale structure on $\sim$ 100 Mpc scales, adds scatter but not bias to \dmhm, and is quantified by $\sigmacosmic(z)$. Dispersion-measure ray tracing through the large-scale structure in Illustris TNG simulation quantifies $\sigmacosmic(z) \approx 60$ pc cm$^{-3}$ of scatter at $z = 0$ and 105 pc cm$^{-3}$ at $z = 0.2$~\citep{walker2024dispersion}. We account for this in each \dmh\ measurement by adding $\sigma_\textrm{IGM}(z_h)$ interpolated between these measurements as a function of redshift. By default we prescribe data errors in Eq.~\ref{eq:err_budget} using \sigmacosmic\ from IllustrisTNG. However, these figures differ by $\sim 20\%$ in the Illustris and EAGLE simulations  \citep{jaroszynski2020frbs,batten2021cosmic}. We check later (see our ``Varying $\sigmacosmic$'' analysis) that our results are insensitive to our choice of $\sigmacosmic$.
    ...
    \item Similarly, FRBs 20200120E~\citep{bhardwaj2021nearby, kirsten2022repeating} and 20240209A\citep{shah2024repeating}, which have among the largest host-galaxy offsets within the FRB population, are excluded from our measurement since they systematically underestimate the average host galaxy CGM (large offset and elliptical, gas-poor host).
\end{enumerate}
...
\begin{equation}
    \sigma_{i}^2 = \sigmacosmic^2 + (0.2 \dm^2_\mathrm{i,NE2001})
\end{equation}
...
It is likely that $\dmhm$ overestimates -- i.e., is a biased estimator of -- $\dmhcgm$.
