...
Although the kinetic Sunyaev-Zeldovich effect lacks this explicit temperature dependence, it is suppressed by a factor of $v/c$, making for a fainter signal that is detected primarily in cross-correlations with massive halos~\citep{schaan2021atacama,hadzhiyska2024evidence}.
...
We introduce and explore the $M^*-\dmh$ relation for isolated star-forming FRB host galaxies at $z < 0.2$, in analogy to correlations between observables (``cluster scaling relations'') which were predicted and observed in more massive halos~\citep{giodini2013}.
...
We show by ray-tracing snapshots in the CAMELS suite of hydro-simulations that the $M^*-\dmh$ relation is a sensitive probe of stellar and AGN feedback models.
...
CAMELS is a suite of small-volume (25 Mpc/h)$^3$ hydrodynamical simulations implementing three distinct subgrid ``prescriptions'' for the unresolved small-scale physics that regulates galaxy formation and evolution, such as star formation and AGN feedback, as implemented in SIMBA, IllustrisTNG, and Astrid hydrodynamical simulations.
...
Cosmological inference from weak lensing will likely need independent gas probes to simultaneously constrain the suppression factor $S(k,z)$. \citet{medlock2025constraining} recently showed with CAMELS that FRB statistics can indeed predict the degree of suppression of the matter power spectrum due to baryonic feedback.
...
Therefore, we choose $b_{\rm min} = 0.13$ in our fiducial analysis to avoid the \dmhism\ contribution entirely on the inner side, and choose $b_{\rm max} = 0.2$.
Our fiducial choice of $b_{\rm min}$ and $b_{\rm max}$ is conservative in the sense that larger values of $b_{\rm min}$ and $b_{\rm max}$ decrease $\widehat{\dmsnap}_\mathcal{M}$.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{msdm_0.10_086.pdf}
...
In addition, to guide the eye, we color each analysis variant (row in Fig~\ref{fig:model}) according to $\log_{10}(K)$, where $K$ compares each of the three models with the most probable of the three feedback models. The odds ratios suggest that the fiducial model of Astrid is ruled out.
    }
    \label{fig:model}
\end{figure*}
...
The fractional discrepancy between the mean and median values of $\dmcosmic(z)$ is $\approx25\%$ at these redshifts, so we vary the amount of $\langle\dmcosmic\rangle$ subtracted by $25\%$ in either direction.
Even this modification does not change our results.
This is consistent with the low redshift of our sample, which guarantees that the Poisson rate of the intervening $L^*$ halos is $<<1$~\citep{mcquinn2014locating}.
...
We can do so by plotting the implied matter power spectrum suppression factor, defined as $S(k) \equiv P_{\rm hydro}/P_{\rm nbody}$, implied by Astrid, IllustrisTNG, and SIMBA evaluated at $z = 0$ in Fig.~\ref{fig:sk}, as well as the baryonic suppression factor~\cite{chisari2018impact} measured in cosmic shear and extrapolated to $z = 0$~\citep{chen2023constraining,terasawa2025exploring} .
