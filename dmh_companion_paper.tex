...
Our stellar masses derived using this method agree with the linear relation between $M^*$ and r-band light ($M_r$) characterized in~\citet{mahajan2018galaxy} deviate at most by 0.5 dex over the range $10^8 < M^*/M_\odot < 10^{11}$. This is consistent with the scatter on that relation ($\pm 0.45$ dex at 1$\sigma$).
...
We denote this theory prediction as $\widehat{\dm}(M^*,\mathcal{M})$ as a function of feedback model $\mathcal{M}$.
...
Our fiducial choice of $b_{\rm min}$ and $b_{\rm max}$ is conservative in the sense that larger values of $b_{\rm min}$ and $b_{\rm max}$ decrease $\widehat{\dm}_\mathcal{M}$.
