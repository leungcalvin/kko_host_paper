...
\begin{align}
    ...
    \mathcal{L}^\mathrm{ul}_{ij} &= \dfrac{1}{2}\left(1 + \mathrm{erf}\left(\dfrac{\dm_\mathrm{h,i} - \widehat{\dm}_j}{(\sigma_i^2 + S_j^2)^{1/2}} \right)\right)
\end{align}
The likelihood from each individual FRB and mass bin is combined via $\log \mathcal{L} = \sum_{i,j} \log\mathcal{L}_{ij}$. 
We calculate the likelihood for each feedback model under our fiducial model parameters.
Also, we vary parameters including $b_{min},b_{max}$, the redshift of our snapshot $(z = 0.0,0.1,0.2)$, and the halo weights $w_k$. 
