...
However, current probes mainly target massive halos: X-ray and thermal Sunyaev-Zeldovich observations, for instance, are temperature sensitive and are biased towards groups and clusters ($M_h \sim 10^{14-15}$)~\citep[e.g.][]{harrison2024observational}.  
...
It is possible to compare our results with existing measurements of baryonic feedback from weak lensing analyses such as cosmic shear. 
We can do so by plotting the implied matter power spectrum suppression factor $S(k)$ implied by Astrid, IllustrisTNG, and SIMBA evaluated at $z = 0$ in Fig.~\ref{fig:sk}, as well as the baryonic suppression factor~\cite{chisari2018impact} measured directly~\citep{chen2023constraining,terasawa2025exploring} in cosmic shear evolved to $z = 0$.
At face value, our results rule out weak feedback (Astrid) and favor the stronger feedback seen in IllustrisTNG and SIMBA (depending on whether \dmh\ is interpreted as a measurement or an upper limit on \dmhcgm respectively).

However, this comparison is subject to several major caveats. 
First, our measurements are from star forming galaxies at $z = 0$ and $M_h \sim 10^{12}$, and FRB hosts show little to no sign of AGN activity -- therefore our measurements  primarily probe the impact of stellar feedback.
...
Instead it is thought that stellar feedback modulates the efficiency of AGN feedback~\citep{medlock2025constraining}; together they drive the expulsion of gas from intermediate-mass $\sim 10^{13.5} M_\odot$ halos.
