...
\newcommand{\dmsnap}{\ensuremath{{\rm DM}_\textrm{snap,i}}}
...
We show by ray-tracing snapshots in the CAMELS suite of hydro-simulations that the $M^*-\dmh$ relation is a sensitive probe of stellar and AGN feedback models~\citep{camels_data_release, camels_presentation, camels_data_release2,ni2023camels}.
...
Our stellar masses derived using this method are in agreement with the linear relation between $M^*$ and r-band light ($M_r$) characterized in~\citet{mahajan2018galaxy} to within 0.5 dex over the range $10^8 < M^* < 10^{11}$ to within the $\pm 0.45$ dex scatter on that relation.
...
The difficulty in measuring the CGM component of FRB host galaxies comes from unmodeled contributions along the line of sight. 
Each \dmh\ measurement is associated with some uncertainty which is dominated by astrophysical (not measurement) uncertainties. 
...
It is likely that there exists a residual upward bias, that is, it is likely that \dmh > \dmhcgm\ for a sightline chosen at random from our subsample. 
However, it is highly unlikely that the bias in any individual measurement exceeds the scatter from the IGM (\sigmaigm=80 pc cm$^{-3}$).
In this way, the stacked sample provides robust upper limits on the circumgalactic dispersion measure of isolated star-forming, $\sim L^*$ FRB hosts in the local Universe. 
...
\section{Constructing the Simulated $M^*-DM_{CGM}$ relation}
To connect the observed $M^*-\dmh$ distribution to galaxy formation, we compare the \dmh-stellar mass relation to predictions from the CAMELS suite of hydrodynamical simulations \citep{camels_data_release, camels_presentation, camels_data_release2}. 
CAMELS is a suite of small-volume (25 Mpc/h)$^3$ hydrodynamical simulations implementing three distinct subgrid ``prescriptions'' for the unresolved small-scale physics that regulates galaxy formation and evolution, such as star formation and AGN feedback, as implemented in SIMBA, IllustrisTNG, and Astrid suites of hydrodynamical simulations.
...
Cosmological inference from weak lensing will likely need independent gas probes to simultaneously constrain the suppression factor $S(k,z)$. \citet{medlock2025constraining} recently showed with CAMELS that FRB statistics indeed can predict the degree of suppression of the matter power spectrum due to baryonic feedback.

From the simulation snapshots it is possible to make theory predictions for the \dmh-$M^*$ relation.
We denote this prediction as $\widehat{\dm}(M^*,\lambda)$ as a function of feedback model $\lambda$.
...
Doing so allows us to conclude that for a given feedback model $\lambda$ that is no value of \dmhcgm\ which satisfies $\dmh > \dmhcgm > \widehat{\dm}_\lambda$.
We following the procedure developed in~\citet{medlock2024probing}, slightly modified to guarantee that $\dmhcgm > \widehat{\dm}_\lambda$.
...
We use \texttt{yt}\citep{yt} to interpolate the electron density field onto a regular grid. 
The electron density field is then integrated along one of the grid axes to make two-dimensional maps of the DM integrated through the snapshot, whose values we refer to as $\dmsnap$.
The lines of sight through the box are binned into categories based on their proximity to haloes in the catalog as quantified by their normalized impact parameter $b = r_\perp/R_{200}$. 
Sightlines intercepting more than one halo within $R_{vir}$, like in criteria 2 in \S\ref{sec:debias}, are discarded as contaminated by groups/clusters.
The remaining sightlines are either ``one-halo'' or ``zero-halo'' sightlines.

The ``zero-halo'' sightlines which do not pass within 0.1 $R_{vir}$ of a halo are used to measure the IGM, which is found by~\citet{medlock2024probing} to be negligible. 
Owing to the small box size, the contribution from the IGM within a snapshot never exceeds 10 pc cm$^{-3}$.

The ``one-halo'' sightlines passing within $b_{min} < R_{vir} < b_{max}$ of a halo are meant to closely simulate our data.
One-halo sightlines often pass through the same halo, so we select at random one sightline per distinct halo in the catalog, discarding all the rest.
This corrects for a preferential sampling of massive halos proportional to their 2d-projected area.
While most FRBs are found in reality to lie within the stellar disk of their host galaxies~\citep[$b \lesssim 0.05$;][]{heintz2020host}, we choose $b_{min} = 0.1$ to avoid the \dmhism\ contribution entirely, which remains barely unresolved at the resolution of CAMELS.
All $\dmsnap$ values are binned by stellar mass, and divided by two to account for the fact that on average, sightlines only pass through half of the host halo.

For a given sub-grid model, each $\dmsnap$ within a stellar mass bin $J$ is averaged, both with and without a star-formation-rate weighting $w_i \propto \mathrm{SFR}$ of the halo associated with sightline $i$.
We refer to the averaged (or ``stacked'') prediction for model $\mathcal{M}$ in the $j$th mass bin as 
\begin{equation} 
    \widehat{\dm}_\mathcal{M} = \sum_{i \in J} w_i \dmsnap^{\mathcal{M}}.
\end{equation}

In this way, our choice of $b_{max} = 0.2$ is conservative: larger $b_{max}$ decreases $\widehat{\dmsnap}_\mathcal{M}$.
However, we have confirmed that changing $b_{min} = 0.1 \to 0.12 \to 0.15$ does not affect our model predictions to within the standard deviation within each mass bin $\sigma_\textrm{snap}$.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{msdm_0.16_090.pdf}
...
In Fig.~\ref{fig:camels} we have plotted the model predictions on top of the measured $M^*-\dmh$ relation. 
Even though our sample was constructed to satisfy $\dmh > \dmhcgm > \langle \dmsnap \rangle$, we see the opposite behavior at $M^* \sim 10^{10-11}$ where in the ASTRID simulations, $\langle \dmsnap \rangle > \dmh$.

\section{Model Selection}
Of the three simulation suites considered, ASTRID has the ``weakest'' feedback -- as pointed out in~\citet{ni2023camels}, AGN kinetic feedback in ASTRID is activated in a more stringent way and with a lower maximum efficiency than in the IllustrisTNG model.
We aim to statistically quantify the challenge to ASTRID using Bayesian model selection.
In this formalism, given that one of these three models -- ASTRID, IllustrisTNG, and SIMBA -- describes reality, we may compute the Bayes factor for each feedback model to establish a hierarchy.

The Bayes factor comparing two models is the ratio of their posterior probabilities; with a prior probability $\pi$ that is uniform between the three models, the Bayes factor between any two models A and B is
\begin{equation}
    K = \dfrac{P(\dmh|\langle \dmsnap^A \rangle}{P(\dmh|\langle \dmsnap^B \rangle)}
\end{equation}
...
We would like to thank Alexa Gordon for providing inclination measurements for FRB 20240201A and 20240310A.
