...
\begin{abstract}
Low-redshift FRBs provide the tightest and most robust constraints on the contents of the circumgalactic medium of $L^*$ galaxies via the inferred host component of the dispersion measure (DM) budget. 
...
We stack a sample of 24 FRBs with robust host associations, selected from publicly-available catalogs to minimize all contributions to \dmh\ except for that from their circumgalactic medium.
...
This reaches halo masses inaccessible by X-ray and Sunyaev-Zeldovich CMB distortions, demonstrating the unique complementarity of FRBs to other gas probes.
...
Following~\citet{medlock2024probing} we define \dmcgm\ to include all contributions to the DM between the spatial extent of the stellar disk and $3R_{200c}$.
...
We fit the SED of these sources using CIGALE~\citep{boquien2019cigale} with a Chabrier IMF~\citep{chabrier2003galactic} \textbf{assuming a delayed-$\tau$ star formation history.}
...
To connect the observed $M^*-\dmh$ distribution to galaxy formation sub-grid models, we compare the \dmh-stellar mass relation to predictions from the CAMELS suite of hydrodynamical simulations \citep{camels_data_release, camels_presentation, camels_data_release2}. 
...
We denote this prediction as $\widehat{\dm}(M^*,\mathcal{M})$ as a function of feedback model $\mathcal{M}$.
...
We following the procedure developed in~\citet{medlock2024probing}, slightly modified to guarantee that $\dmhcgm > \widehat{\dm}_\mathcal{M}$.
...
We select sightlines whose impact parameters lie within some annulus $b_{min} < b < b_{max}$ around a halo.
...
In reality, most FRBs are found to lie within the stellar disk of their host galaxies~\citep[$b \lesssim 0.05$;][]{heintz2020host}.
Therefore, we choose $b_{min} = 0.13$ in our fiducial analysis to avoid the \dmhism\ contribution entirely on the inner side, and choose $b_{max} = 0.2$.
Our fiducial choice of $b_{min}$ and $b_{max}$ are conservative in the sense that larger values of $b_{min}$ and $b_{max}$ decrease $\widehat{\dmsnap}_\mathcal{M}$.
However, we will later explore variations in both $b_{min}$ and $b_{max}$.
After restricting sightlines to a range of $b$ values, $\dmsnap$ values are binned by stellar mass, and divided by two to account for the fact that on average, sightlines only measure half of the host CGM.
...
We explore alternative weightings a uniform weighting over all simulated halos ($w_k = 1$) and $w_k \propto M^* \times \mathrm{SFR}$. 
...
In this formalism, given that one of these three fiducial models -- ASTRID, IllustrisTNG, and SIMBA -- describes reality, we may compute the Bayes factor for each feedback model to choose between them.
The Bayes factor $K$ comparing models $\mathcal{A}$ and $\mathcal{B}$ is the ratio of their posterior probabilities. 
...
We write down the likelihood of getting the observed data from an FRB source i whose host galaxy falls in mass bin j. 
...
Then for each likelihood individually, we combine posteriors from each individual FRB and mass bin via $\log \mathcal{L} = \sum_{i,j} \log\mathcal{L}_{ij}$. 

\begin{figure*}
    \includegraphics[width=0.49\textwidth]{model_selection_ul.pdf}
    \includegraphics[width=0.49\textwidth]{model_selection_gauss.pdf}
    \caption{We print the summed log-likelihood for the fiducial analysis and several variants thereof in Fig.~\ref{fig:posterior}. 
...
With the underlying assumption that one of the three fiducial CAMELS models accurately captures the impact of feedback in $L^*$ halos, our observations and characterization of the $M^*-\dmh$ relation provide strong evidence against the feedback model implemented in Astrid, with Bayes factors well in excess of the threshold needed to rule out a model.
...
Our fiducial results use the $z = 0.1$ snapshot, with sightlines selected between $0.13 < b < 0.20$, followed by calculation of the weighted mean and scatter of \dmhcgm\ in several mass bins, where the weights for each sightline in a mass bin are proportional to the star formation rate of that halo.
...
To verify that we are avoiding the host galaxy ISM, we vary $b_{min}$.
To verify that our results are agnostic to the FRB-host galaxy offset distribution, we increase $b_{max}$ to 0.2.
...
The fractional discrepancy between the mean and median value of $\dmcosmic(z)$ is $\approx25\%$ at these redshifts, so we vary the amount of $\langle\dmcosmic\rangle$ subtracted by $25\%$ in either direction.
Even this modification does not change our results.
This is consistent with the low redshift of our sample, which guarantees that the Poisson rate of intervening $L^*$ halos is $<<1$~\citep{mcquinn2014locating}.
...
It is possible to compare our results on equal footing with existing measurements of baryonic feedback from weak lensing analyses such as cosmic shear. 
We can do so by plot the implied matter power spectrum suppression factor $S(k)$ implied by Astrid, IllustrisTNG, and SIMBA evaluated at $z = 0$ in Fig.~\ref{fig:sk}, as well as the baryonic suppression factor~\cite{chisari2018impact} measured directly~\citep{chen2023constraining,terasawa2025exploring} in cosmic shear evolved to $z = 0$.
At face value, our results rule out weak feedback (ASTRID) and favor the stronger feedback seen in IllustrisTNG and SIMBA (depending on whether \dmh\ is interpreted as a measurement or an upper limit on \dmhcgm).

However, this comparison is subject to several major caveats. 
First, our measurements are sensitive to star forming galaxies at $z = 0$ and $M_h \sim 10^{12}$; FRB hosts show little to no sign of AGN activity.  
This makes our data very complementary to weak lensing.
DES weak lensing, for instance, is sensitive to $z \sim 0.5$ and $M_h \sim 10^{14}$, whereas Subaru-HSC reaches $z \sim 1$.
At those halo masses, gas expulsion at low redshifts is unlikely to be driven by stellar feedback alone.
Instead it is thought that stellar feedback modulates the efficiency of AGN feedback; together they drive the expulsion of gas from intermediate-mass $\sim 10^{13.5} M_\odot$ halos.
...
It can also be extended to higher redshifts to study the evolution of feedback over cosmic time, although the foreground posed by the IGM will demand larger number of sources ($\sigmaigm$ increases).
