...
We define \dmhcgm\ to include all contributions to the DM integral between the spatial extent of the stellar disk and $\sim 7.5$ Mpc outside the galaxy. This definition is motivated by the dimensions of the CAMELS snapshots (15 Mpc). We argue that this definition is better-suited for comparisons between simulated and observed FRB sightlines. It is more comprehensive of circumgalactic electrons than, for instance, a definition which fix a boundary between the CGM and IGM at some multiple of $R_{200c}$. Those definitions which invoke a dependence on halo mass, which is nontrivial to measure. The trade-off is that this definition incurs a bias due to the IGM contribution integrated over a $15$ Mpc sightline. However, this background contribution from the IGM contribution over 15 Mpc is small compared to the relevant uncertainties ($\sigmacosmic^2 + S_j^2$, defined later), and can be independently characterized.

We introduce and explore the $M^*-\dmh$ relation for isolated star-forming FRB host galaxies at $z < 0.2$, in analogy to correlations between observables (``cluster scaling relations'') which were predicted and observed in more massive halos~\citep{giodini2013,battaglia2012on}.
...
It makes our results insensitive to redshift evolution effects and the cosmic contribution, whose scatter $\sigmacosmic$ will add extra noise in our measurement of $\dmhcgm$ which increases with increasing redshift~\citep{batten2021cosmic,walker2024dispersion}. 
...
For each FRB, we calculate $\dmhm$ according to Eq.~\ref{eq:dmh_definition}, using the NE2001~\citep{cordes2002ne2001} model to estimate $\dmmwism$ and the~\citet{yamasaki2020galactic} model to estimate $\dmmwcgm$, and the open-source\footnote{\texttt{https://github.com/FRBs}} implementation of $\langle \dmcosmic \rangle$.
...
\begin{figure*}
    ...
    \caption{Measurements of $\dmhm$, as a function of $r$-band host galaxy luminosity for low-redshift FRBs in the literature. Y-axis error bars indicate the uncertainty in $\dmmwism$, taken to be $20\%$ of the NE2001 vaue. The ``Cluster'' and ``Edge on'' labels identify host galaxies associated with known galaxy clusters or highly-inclined host galaxies (see text). The ``Scattered'' label refers to sources with scattering timescales above 5 ms, which are removed (see text).}
    \label{fig:host_dm}
\end{figure*}
...
CAMELS is a suite of small-volume (25 Mpc/h)$^3$ hydrodynamical simulations implementing \msbold{several} distinct subgrid ``prescriptions'' for the unresolved small-scale physics that regulates galaxy formation and evolution, such as star formation and feedback and AGN activity. 
...
Finally, we check that our results are also insensitive to the threshold used in PATH, the industry-standard algorithm by which FRBs are associated with their hosts~\citep{aggarwal2021probabilistic}, which is known to select for bright, extended galaxies.

\section{Discussion and Conclusion}
In this paper, we have explored the $M^*-\dmh$ relation for the first time, showing that it diverges as a function of feedback model in isolated galaxies $M^* \sim M^*_\mathrm{MW}~(M_h \sim 10^{12})$. We have used it to constrain the gas environments of isolated star-forming galaxies at $z = 0$ and $M_h \sim 10^{12}$ -- a mass range lower than accessible through SZ and X-ray observations. 

Of the three fiducial simulations considered, our observations most strongly challenge Astrid, which is considered to have the ``weakest'' feedback. Our Bayesian model selection shows that this conclusion is insensitive to a range of astrophysical assumptions.
