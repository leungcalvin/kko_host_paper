...
One of the earliest landmarks in observational FRB science used the Australian Square Kilometre Array Pathfinder (ASKAP)~\citep{macquart2020census} to locate the missing baryons to the intergalactic medium (IGM). That result established that of all contributions to the total dispersion measure (DM) of an FRB, the mean contribution from the intergalactic medium out to the redshift of the host galaxy $z_h$ is the dominant contributor to the DM of an FRB beyond $z_h \sim 0.2$, establishing what FRB astronomers now refer to as the ``Macquart relation''. 
...
Previous efforts have relied on a ``wedding-cake'' combination of multi-object spectroscopic surveys to disentangle the contributions of the host and interlopers to the overall DM budget of FRBs~\citep{khrykin2024flimflam}. In this paper we aim to isolate the contribution of the circumgalactic medium of the FRB host to the dispersion measure budget by making cuts on the growing sample of localized FRBs in the literature, which now exceeds 100 sightlines. The circumgalactic contribution is a significant contributor to, but distinct from, the host contribution in its own rest frame. The former has physical significance as a characterization of the gas column surrounding a galaxy, but cannot easily be measured from data. The latter can readily be measured from data (\dmh in Eq.~\ref{eq:dmh_definition}), but only constitutes an upper limit on the former. We have
\begin{equation}
...
Here the subscripts ``tot'', ``cosmic'', and ``MW'' indicate dispersion measures as measured in data, the mean cosmic contribution out to redshift $z_h$, and the Milky Way (disk plus halo) contributions of the DM respectively. Here and throughout this paper, the angle brackets refers to an average over all sightlines out to that redshift $z_h$, i.e. neglecting the effect of large-scale structure and interlopers. The circumgalactic contribution is non-trivial to disentangle from \dmh as defined in Eq.~\ref{eq:dmh_definition}, as it contains poorly-characterized contributions from large-scale structure, the circum-source medium, and the host galaxy ISM. In  \S~\ref{sec:sample} we describe how we minimize the aforementioned sources of contamination in our measurement by selecting a clean sample of FRB sightlines. We use publicly-available FRBs in the literature in addition to a recently published catalog of low redshift FRBs, presented in a companion work~\citep{chime2024catalog}. 
In \S~\ref{sec:moments} we describe our measurement of \dmh\ using our clean sightlines. We characterize the distribution of \dmh\ via its moments $\langle \dmh \rangle$, as well as higher moments thereof ($\langle \dmh^2 \rangle$, etc.). These moments non-parametrically specify the sightline-to-sightline variation in $\dmh$, and contain information about the spatial distribution of ionized gas relative to that of FRB hosts~\citep{cordes2016radio, reischke2024analytical}. Finally, in \S~\ref{sec:camels} we introduce for the first time in the literature the \dmh-stellar mass relation and compare it to theoretical predictions from three sub-grid models implemented in the CAMELS suite of hydrodynamical simulations to constrain models for star formation and galactic winds originating from e.g. supernova and AGN feedback in isolated $L^*$ galaxies. This corresponds to a halo mass range inaccessible to traditional gas probes, i.e. X-ray emission and Sunyaev-Zeldovich observations. All quantities in this work are calculated using the \textit{Planck} cosmology for a flat $\Lambda$CDM universe of  $H_0 = 67.7~\mathrm{km~s}^{-1}~\mathrm{Mpc}^{-1}$, $\Omega_m=0.31$ and $\Omega_\Lambda = 0.69$ \citep{collaboration2020planck}. 
...
Finally, to connect our characterization of the \dmh\ distribution to galaxy formation and cosmology, we compare the \dmh-stellar mass relation to predictions from the CAMELS suite of hydrodynamical simulations. CAMELS is a suite of small-volume (25 Mpc/h)$^3$ hydrodynamical simulations implementing several different artificial ``prescriptions'' for the unresolved small-scale physics that regulates galaxy formation and evolution, such as star formation and AGN feedback. One major goal of the CAMELS project is to produce theory predictions to independently constrain the suppression of the matter power spectrum induced by baryonic feedback relative to a dark-matter only simulation, as measured in current and next-generation weak lensing surveys such as the Dark Energy Survey~\citep{chen2023constraining,aric_o2023des} and LSST/Rubin. Cosmological inference from weak lensing will likely need independent gas probes to simultaneously constrain the suppression factor $S(k,z)$.
...
We start with both halo catalogs and simulation snapshots from the the \texttt{1P\_p7\_0} set of simulations in CAMELS. We use \texttt{yt}\citep{yt} to interpolate the electron density field onto a regular grid. The electron density field is then integrated along one of the grid axes to make two-dimensional electron maps. Lines of sight are binned into categories based on their proximity to haloes in the catalog. Sightlines intercepting more than one halo within $R_{vir}$ are discarded. Each remaining sightline either passes within 0.1 $R_{vir}$ of a halo in the catalog, or it does not. 

The ``one-halo'' sightlines passing within 0.1 $R_{vir}$ of a halo are similar to typical FRB sightlines in our sample, which extend through the IGM and terminates in the stellar disk of its isolated host galaxy at an impact parameter which is some small fraction of $R_{vir}$. Similarly, the line-of-sight integral is terminated once the sightline reaches $0.1 R_{vir}$ away from the halo's catalog position. This captures the CGM contribution to \dmh\ while avoiding the ISM of the host galaxy. In the same way that real FRB sightlines with interlopers within $R_{vir}$ are discarded, we throw away sightlines with interlopers within $R_{vir}$ in the simulations. 
