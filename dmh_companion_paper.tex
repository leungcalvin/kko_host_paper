...
Long before the first repeating fast radio burst (FRB) was localized to a cosmologically-distant host galaxy~\citep{chatterjee2017direct,tendulkar2017host},~\citet{mcquinn2014locating} realized the immense potential of FRBs as cosmological probes of the cosmic baryons.
This is because FRBs have dispersion measures integrate the column density of electrons between the observer and the FRB source $z_F$.

After subtracting the DM from the Galactic ISM, the extragalactic DM can be its host, and intervening contributions such as diffuse IGM and the CGM of intervening galaxies.
\begin{equation}
    \dm = \int_{0}^{z_F} n_e(z)~dz = \dmmwism + \dmmwcgm + \dmcosmic(z) + \underbrace{\dmhcgm + \dmhism + \dmcbm}_{\dmh}
\end{equation}


Since then, there have emerged two classes of methodologies used to study the diffuse extragalactic baryons using FRBs.


The first class studies the diffuse IGM by analyzing the the Australian Square Kilometre Array Pathfinder (ASKAP)~\citep{macquart2020census} to locate the missing baryons to the intergalactic medium (IGM). 
That result established that the mean contribution from the intergalactic medium is the dominant contributor to the DM of an FRB for sources beyond $z \sim 0.2$, establishing what FRB astronomers now refer to as the ``Macquart relation''. 
An implied conclusion of~\citet{macquart2020census}, whose eponymous relation now extends to $z =  1.3$~\citep{connor2024gas}, is that the contribution from the host galaxy is typically subdominant to that of the IGM. 

The second class of study traces the collapsed baryons by subtracting the estimated average cosmic dispersion measure out to the redshift of the host galaxy of the FRB source.
Nevertheless, the host galaxy DM contribution (hereafter, \dmh) is worthy of dedicated study: it is crucial to virtually all attempts to forward model FRB dispersion measures as probes of ionized gas~\citep{james2021z,james2022measurement,rafiei_ravandi2021chime,2019arXiv190102418M}, and contains valuable information about the circumgalactic medium (CGM) and broader gas ecosystem in which FRB hosts reside\citep{prochaska2019probing}.

The first is forward-modeling of 
Previous efforts to study galactic halos using FRBs have relied on a ``wedding-cake'' combination of multi-object spectroscopic surveys to decompose the overall DM budget of FRB sightlines into contributions from intervening galaxies and the halo of the host galaxy~\citep{khrykin2024flimflam}.
Those optical observations are sensitive to both the ``two-halo'' and ``one-halo'' contributions, but the scatter in the total DM due to the large-scale structure adds significant noise which is larger than the signal from individual $L*$ haloes.
In this paper, we instead focus instead on the latter by stacking a sample of 29 low-redshift FRBs, whose sightlines lack contributions from intervening galaxies.
...
\section{Subgrid Model Selection}


Since the vast majority of FRBs originate within the stellar disk of their hosts, the so-called ``host'' contribution \dmh\ (as defined in Eq.~\ref{eq:dmh_definition}) always picks up a contribution from the circumgalactic medium of the host. However, the circumgalactic component is often swamped by other contributions to the DM budget which are hard to independently characterize, similar in size to the CGM contribution, and which likely vary from sightline to sightline. These include contributions from large-scale structure, the circum-source medium, and the ISM of the host galaxy and the Milky Way. We have
%
\begin{equation}
\dfrac{\dmh}{1 + z_h} = \textrm{DM}_\mathrm{tot} - \langle \textrm{DM}_\textrm{cosmic}(z_h)\rangle - \textrm{DM}_{\textrm{MW}}.
\label{eq:dmh_definition}
\end{equation}

Here the subscripts ``tot'', ``cosmic'', and ``MW'' indicate the total DM as measured in FRB data, the mean cosmic contribution out to redshift $z_h$, and the Milky Way (disk plus halo) contributions of the DM respectively. Here and throughout this paper, the angle brackets refer to taking an average over all sightlines. 

In  \S~\ref{sec:sample} we describe how we minimize the aforementioned sources of contamination in our measurement by selecting a clean sample of FRB sightlines.
In \S~\ref{sec:moments} we describe our measurement of \dmh\ using our clean sightlines. Finally, in \S~\ref{sec:camels} we introduce for the first time in the literature the \dmh-stellar mass relation and compare it to theoretical predictions from three sub-grid models implemented in the CAMELS suite of hydrodynamical simulations to constrain models for star formation and galactic winds originating from e.g. supernova and AGN feedback in isolated $L^*$ galaxies. This corresponds to a halo mass range inaccessible to traditional gas probes, i.e. X-ray emission and Sunyaev-Zeldovich observations. All quantities in this work are calculated using the \textit{Planck} cosmology for a flat $\Lambda$CDM universe of  $H_0 = 67.7~\mathrm{km~s}^{-1}~\mathrm{Mpc}^{-1}$, $\Omega_m=0.31$ and $\Omega_\Lambda = 0.69$ \citep{collaboration2020planck}. 

\section{Sample Selection}\label{sec:sample}
We select a subsample of FRBs available in the literature to reduce contaminants for our measurement of \dmh, out of all FRBs in the literature with secure redshifts and publicly-available photometry. We perform a redshift cut, eliminating sources beyond $z > 0.2$. This boundary is chosen to balance a trade off: we have statistics from including more distant sources, but at lower redshifts, we are less subject to optical selection, in which host galaxies significantly less luminous than $L^*$ may remain undetected.~\citep{seebeck2021effects,marnoch2023unseen,jahnsschindler2023how}, and insensitive to the common assumption that $\langle \dmh \rangle$ is constant over cosmic time.
