...
We study the relationship between FRB host stellar masses \mstar\ and their host dispersion measures \dmh\ and use the $M^*-\dmh$ relation to constrain models for feedback in isolated galaxies as implemented in the CAMELS suite of hydrodynamical simulations.
...
Historically, this quantity has been referred to as the ``host contribution'' to the DM budget and is denoted \dmh. It may be estimated by subtracting estimates for the cosmic web
\avgdmcosmic~\citep[see e.g.][]{simha2020disentangling} and the Galaxy \dmmw=\dmmwcgm + \dmmwism\
...
We have validated that the stellar masses derived using this method are consistent with the linear relation between $M^*$ and r-band light ($M_r$) characterized in~\citet{mahajan2018galaxy}. They differ by at most 0.5 dex over the range $10^8 < M^*/M_\odot < 10^{11}$, which is consistent with the scatter on that relation ($\pm 0.45$ dex at 1$\sigma$). Furthermore, the CIGALE and NED-LVS mass estimates are consistent within fitting uncertainties for the two sources for which both data are available.
...
\begin{enumerate}
    \item Host galaxies which are embedded in groups and clusters have contributions to the total DM on $\sim$ Mpc scales which add a large positive bias to \dmhm which may contribute several hundred pc cm$^{-3}$. Galaxy cluster FRBs have significant contamination to \dmh\ which are not associated with the circumgalactic medium of their hosts. Galaxy cluster FRBs include FRB 20220914A and FRB 20220509G~\citep{connor2023deep}, FRBs 20231206A, 20230203A, and 20231204A~\citep{chime2025catalog}. These account for some of the largest $\dmh$ data points in Fig.~\ref{fig:host_dm} and are thus excluded from our analysis.
    ...
    \item So-called ``low-DM'' FRB sightlines. For surveys with large ($\gtrsim 1'$) localization regions, the low total DM can be used to make a host galaxy association, but only by imposing a distance cutoff. However, these associations bias the sample because of the explicit selection bias on DM$_{tot}$ and therefore $\dmh$. To estimate the host CGM contribution, which is upper bounded by~\dmh, it is essential to exclude low-DM selected sightlines from the clean sample, since they are not representative estimates of $\langle \dmh \rangle$. There are nine FRB hosts identified this way described in~\citet{bhardwaj2024host,michilli2023subarcminute,Ibik2024host}: They include FRBs 20180814A, 20181030A, 20181220A, 20181223C, 20190418A, 20190110C, 20190425A, 20200223B, as well as 20190303A (note its repeat burst FRB 20231204A). Analyzing this sub-sample separately confirms that these bursts are drawn from the low-\dmhm\ tail of the FRB population.
    \item Similarly, FRBs 20200120E~\citep{bhardwaj2021nearby, kirsten2022repeating} and 20240209A\citep{shah2024repeating}, which have among the largest host-galaxy normalized offsets within the FRB population, are excluded from our measurement since they systematically underestimate the average host galaxy CGM (large offset and elliptical, gas-poor host).
\end{enumerate}

Applying these cuts leaves 24 bursts (12 in the range $z < 0.1$, 12 in the range $0.1 < z < 0.2$) (Table~\ref{tab:sample}).
For these bursts, a less-biased estimator of \dmhcgm. 
...
\begin{equation}
    \sigma_{i}^2 = \sigmacosmic(z)^2 + (0.2 \dm^2_\mathrm{i,NE2001}).
\end{equation}
...
In general, \sigmacosmic(z) varies as a function of feedback model, which we are trying to constrain.
...
We use the IllustrisTNG values to define our fiducial uncertainties, but we test the impact of varying \sigmacosmic\ as a function of feedback model.
...
Our \dmhcgm\ predictions are slightly sensitive to the host offset distribution of FRBs: FRB sources highly offset from their gas halos $(\langle b^2 \rangle \gtrsim 0.5)$ may exclude a portion of \dmhcgm\ from their sightlines. Though high-offset exceptions are known to exist~\citep{kirsten2022repeating,shah2024repeating,eftekhari2024massive}, it is thought that this does not happen for the majority of FRB sources which are robustly associated with their hosts~\citep[$b \lesssim 0.05$;][]{heintz2020host}.

After restricting the simulation sightlines to a range of $b$ values, $\dmsnap$ values are binned by stellar mass and divided by two to account for the fact that observed FRB sightlines skewer halfway through their host halos on average.
...
We write down the likelihood of getting the observed data from an FRB source i whose host galaxy falls in a mass bin j. If \dmh\ is an unbiased estimator, then a Gaussian likelihood is most appropriate. 
\begin{align}
P^\mathrm{gauss}_{ij} = \mathcal{L}^\mathrm{gauss}_{ij} &= \exp\left[ -\dfrac{1}{2}\dfrac{(\dm_{\mathrm{h,i}} - \widehat{\dm}_j)^2}{\sigma_i^2 + S_j^2} \right]\\
\intertext{However it is much more likely that \dmh\ is a biased estimator which overestimates \dmhcgm, in which case it merely places an upper limit on \dmhcgm. The remaining scatter $\dmhcbm$ and bias $\sigma_\mathrm{CBM}$ are likely associated with the circum-burst medium. In this case the posterior probability is given by marginalizing over the true value of $\dmhcgm$ and $\sigma_\mathrm{CBM}$ compatible with the data.}
P^\mathrm{ul}_{ij}\mathcal{L}^\mathrm{ul}_{ij} &= \int_0^{\infty}~\pi(\sigma) \dfrac{1}{2}\left(1 + \mathrm{erf}\left(\dfrac{\dm_\mathrm{h,i} - \widehat{\dm}_j}{2(\sigma_i^2 + S_j^2 + \sigma^2_\mathrm{CBM})^{1/2}} \right)\right)~d\sigma
\end{align}
...
We explore different weighting schemes which allow FRB sources to trace either the star formation rate, stellar mass ($w_h \propto M^*$), both ($w_h \propto \mathrm{SFR} M^*$), or neither property ($w_h = 1$ for every halo).

We also investigate the possibility that our selection of isolated FRB host galaxies preferentially selects underdense sightlines. In this scenario, $\dmcosmic(z)$ for our sightlines may not be well-predicted by its mean value $\langle\dmcosmic(z)\rangle$ averaged over all sightlines out to a given $z$.
The median sightline is underdense relative to the mean sightline, as the latter gets positively-skewed by the contributions from intervening halos.
The fractional discrepancy between the mean and median values of $\dmcosmic(z)$ is $\approx25\%$ at $z \lesssim 0.2$. We test for this possibility by varying the amount of $\langle\dmcosmic\rangle$ subtracted in Eq.~\ref{eq:dmh_definition} by $25\%$ in either direction for every sightline in our sample.
\msbold{Our conclusions are unaffected in in the upper-limit case, but in the Gaussian case, we find that the strong preference of IllustrisTNG over Astrid is slightly weakened when $\dmcosmic(z)$ is increased by $25\%$. Our insensitivity to this large shift in the cosmic DM can be explained by the low redshift of our sample: at $z < 0.2$, the mean number of intersections within $R_{200c}$ of an $L^*$ halo is $<<1$~\citep{mcquinn2014locating}.}
The ``low-end only'' and ``high-end only'' analysis variants split our sample around the median stellar mass ($10^{9.8} M_\odot$). They suggests that the low end of the $M^*-\dmh$ relation is not very constraining, with the caveat that the limited resolution of CAMELS ($1.3\times 10^7 h^{-1} M_\odot$) may more strongly influence the low-mass end of our theory predictions. Instead, most of the discriminating power comes from the high-mass end of the $M^*-\dmh$ relation, as seen visually in Fig.~\ref{fig:camels}.
Finally, we check that our results are also insensitive to our host associations, by imposing a more stringent association threshold of $0.95 \leq P(O|x)$, where $P(O|x)$ is the posterior probability that the FRB is associated with its putative host, as computed by the industry-standard PATH algorithm~\citep{aggarwal2021probabilistic}.
